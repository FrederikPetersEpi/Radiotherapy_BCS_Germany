...
#Sys.setenv(NLS_LANG = "GERMAN_GERMANY.AL32UTF8")
...
write.table(tab1,"table_1_deutschland_mit_NRW.csv",sep=";")
getwd()
...
sum.tab
write.table(sum.tab,paste(Sys.Date(),'_ci_model.1_ST_mit_NRW.csv',sep=''),sep=';')
...
forest_plot_os = forester(
  ...
  file_path = "Z:/#OFFEN/28-Auswertungen/2024/2024-02_OP_Qualitaet_regional/Analysen_Bund/forest_radiation_mit_NRW.png",
  dpi = 600,
  xlim = c(0.5, 1.5),
...
p=ggplot(bar.1, aes(x=District,y=prob_st_obs))+
  ...
  annotate("text",x=25,y=0.81,label="Germany: 0.74",color="black",hjust=0)+
...
ggsave("proportion_RT_plot_mit_NRW.png",plot=p,width=6,height=4,dpi=600)
getwd()



# Bardiagramm mit Farbverlauf

# Kategorisierungsfunktion verwenden (wie in deinem Karten-Skript)
library(dplyr)
bar.1 <- bar.1 %>%
  mutate(abweichung_prozent = ((prob_st_obs - 0.79) / 0.79) * 100,
         abweichung_kat = case_when(
           abweichung_prozent <= -10 ~ "< -10%",
           abweichung_prozent > -10 & abweichung_prozent <= -5 ~ "-10% bis -5%",
           abweichung_prozent > -5 & abweichung_prozent <= -2 ~ "-5% bis -2%",
           abweichung_prozent > -2 & abweichung_prozent < 2 ~ "±2% vom Median",
           abweichung_prozent >= 2 & abweichung_prozent < 5 ~ "+2% bis +5%",
           abweichung_prozent >= 5 & abweichung_prozent < 10 ~ "+5% bis +10%",
           abweichung_prozent >= 10 ~ "> +10%",
           TRUE ~ "Keine Daten"
         ))

# Farbpalette (wie in deinem Karten-Skript)
farben <- c(
  "< -10%" = "#8B0000", 
  "-10% bis -5%" = "#CD5C5C", 
  "-5% bis -2%" = "#F08080",
...
# Plot mit Farben
p <- ggplot(bar.1, aes(x = District, y = prob_st_obs, fill = abweichung_kat)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = farben, name = "Deviation from\nnational median") +
	  scale_linewidth_manual(values = c("Other" = 0, "Bremen" = 1.2, "Hamburg" = 1.2), 
                         guide = "none") +
  geom_hline(yintercept = 0.79, col = "black", linewidth = 0.8) +
  ylim(0, 1) +
  labs(x = "Districts", y = "Proportion with radiotherapy") +
  annotate("text", x = 25, y = 0.81, label = "Germany: 0.79", color = "black", hjust = 0) +
	 annotate("text", x = 25, y = 0.95, label = "★ = City states (Bremen, Hamburg)", 
...
ggsave("proportion_RT_plot_colored.png", plot = p, width = 8, height = 4, dpi = 600)


# Stelle sicher, dass city_positions existiert
city_positions <- bar.1 %>% 
  filter(kreis_id %in% c(4011, 2000)) %>%
  select(District, prob_st_obs, city_state)

# Prüfe ob Städte gefunden wurden
...
p <- ggplot(bar.1, aes(x = District, y = prob_st_obs, fill = abweichung_kat)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(values = farben, name = "Deviation from\nnational median") +
  # Sterne über den Balken hinzufügen (WICHTIG: inherit.aes = FALSE)
  geom_point(
    data = city_positions, 
    aes(x = District, y = prob_st_obs + 0.03),
    shape = 8,        # Stern
    size = 1,         # Größe
    color = "black",
    inherit.aes = FALSE  # WICHTIG: sonst übernimmt er fill von oben
  ) +
  geom_hline(yintercept = 0.79, col = "black", linewidth = 0.8) +
  ylim(0, 1.05) +  # Wichtig: auf 1.05 erhöhen für die Sterne
  labs(x = "Districts", y = "Proportion with radiotherapy") +
  annotate("text", x = 25, y = 0.815, label = "Germany: 0.79", color = "black", hjust = 0) +
  annotate("text", x = 25, y = 1.02, label = "★ = City states (Bremen, Hamburg)", 
           color = "black", hjust = 0, size = 3) +
  theme_minimal() +
  theme(legend.position = "right")
...
p <- ggplot(bar.1, aes(x = District, y = prob_st_obs, fill = abweichung_kat)) +
  geom_bar(stat = "identity", aes(color = city_state, linewidth = city_state)) +
  scale_fill_manual(values = farben, name = "Deviation from\nnational median") +
  scale_color_manual(
    values = c("Other" = NA, "Bremen" = "black", "Hamburg" = "black"), 
    guide = "none"
  ) +
  scale_linewidth_manual(
    values = c("Other" = 0, "Bremen" = 1.5, "Hamburg" = 1.5), 
    guide = "none"
  ) +
  geom_hline(yintercept = 0.79, col = "black", linewidth = 0.8) +
  ylim(0, 1) +
  labs(
    x = "Districts", 
    y = "Proportion with radiotherapy",
    caption = "City states (Bremen, Hamburg) marked with black border"
  ) +
  annotate("text", x = 25, y = 0.81, label = "Germany: 0.79", color = "black", hjust = 0) +
  theme_minimal() +
  theme(
...
print(p)
ggsave("proportion_RT_plot_colored_with_border.png", plot = p, width = 8, height = 4, dpi = 600)


scale_linewidth_manual(
  values = c("Other" = 0, "Bremen" = 2.0, "Hamburg" = 2.0),  # Dicker!
  guide = "none"
)


scale_color_manual(
  values = c("Other" = NA, "Bremen" = "gold2", "Hamburg" = "gold2"), 
  guide = "none"
)
...
bar.1 <- bar.1 %>%
  mutate(
    city_state = case_when(
      kreis_id == 4011 ~ "Bremen",
      kreis_id == 2000 ~ "Hamburg",
      TRUE ~ "Other"
    )
  )
...
sum.tab$variable=rownames(sum.tab)
sum.tab
write.table(sum.tab,paste(Sys.Date(),'_ci_model.time_to_ST_mit_NRW.csv',sep=''),sep=';')
...
forest_plot_os = forester(
  ...
  display=TRUE,
  estimate_precision = 2,
  file_path = "Z:/#OFFEN/28-Auswertungen/2024/2024-02_OP_Qualitaet_regional/Analysen_Bund/forest_time_to_RT_mit_NRW.png",
  dpi = 600,
  xlim = c(-60, 60),
...
sum.tab$variable=rownames(sum.tab)
sum.tab
write.table(sum.tab,paste(Sys.Date(),'_ci_model_surival_ST_mit_NRW.csv',sep=''),sep=';')
...
forest_plot_os = forester(
  ...
  display=TRUE,
  estimate_precision = 2,
  file_path = "Z:/#OFFEN/28-Auswertungen/2024/2024-02_OP_Qualitaet_regional/Analysen_Bund/forest_survival_mit_NRW.png",
  dpi = 600,
  xlim = c(0, 3),
